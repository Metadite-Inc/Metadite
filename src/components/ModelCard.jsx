...
const ModelCard = ({ model, user, isFavorite: initialIsFavorite, onRemoveFavorite }) => {
  ...
  const getOptimizedImageUrl = (url) => {
    ...
    // For external images, add size parameters if possible
    // This is a basic implementation - in production you'd use a proper image optimization service
    return url;
  };

  const optimizedImageUrl = getOptimizedImageUrl(model.image);
  ...
  return (
    <div className="glass-card rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg transform hover:-translate-y-1">
          ...
          <Link to={`/model/${model.id}`}>
            {!imageError ? (
              <img
                ...
                alt={model.name}
                className={`w-full h-full object-cover transition-all duration-700 hover:scale-110 cursor-pointer ${
      ...
      <div className="p-5">
        ...
        <p className="text-gray-600 dark:text-gray-300 text-sm line-clamp-2 mb-4">{model.description}</p>
